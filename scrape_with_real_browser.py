"""
连接到用户已打开的 Chrome 浏览器进行抓取
这样就是完全真实的人类会话，不会被识别为机器人
"""
import json
import re
import time
...
def main():
    print("=" * 70)
    print("YP US 商户商品批量抓取工具（连接真实浏览器版本）")
    print("=" * 70)
    ...
    print("【重要操作步骤】")
    print("=" * 70)
    print("1. 请用普通浏览器（Chrome/Edge）手动打开:")
    print("   https://www.yeahpromos.com/index/login/login")
    print("2. 登录 YP（正常输入，不会被清空）")
    print("3. 登录成功后，保持浏览器窗口打开")
    print("4. 回到这里按 Enter 继续")
    print("=" * 70)
    ...
    with sync_playwright() as p:
        # 连接到已有的 Chrome 实例（需要 Chrome 以调试模式启动）
        # 先尝试连接到默认端口 9222
        try:
            ...
            print("2. 以调试模式启动 Chrome（复制下面命令到 CMD 运行）:")
            print("   chrome.exe --remote-debugging-port=9222")
            print("3. 在启动的 Chrome 中登录 YP")
            print("4. 回到这里按 Enter，我会重新连接")
            input("\n完成上述步骤后按 Enter 继续...")
            try:
                browser = p.chromium.connect_over_cdp("http://localhost:9222")
                print("✅ 成功连接到浏览器！")
            except:
                print("❌ 仍然连接失败，请确保 Chrome 以调试模式启动")
                return
        ...
        # 验证登录状态
        try:
            page.goto('https://www.yeahpromos.com/index/advert/index', timeout=10000)
            content = page.content()
            if 'Login name cannot be empty' in content or 'Login name' in content:
                print("⚠️ 检测到未登录，请在浏览器中登录 YP")
