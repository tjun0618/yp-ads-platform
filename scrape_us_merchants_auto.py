"""
用 Playwright 批量抓取 APPROVED US 商户（自动检测登录版本）
特点：自动检测登录状态、断点续传、稳定运行
"""
import json
import re
import time
...
def check_login(page):
    try:
        ...
        return 'Login name cannot be empty' not in content and 'Login name' not in content
    except:
        return False

def wait_for_login(page, max_wait_seconds=180):
    print("\n" + "=" * 70)
    print("=== 等待登录 YP ===")
    print("=" * 70)
    print("1. 浏览器窗口已打开登录页")
    print("2. 请在浏览器中输入账号密码登录")
    print(f"3. 脚本会自动检测登录状态（最多等待 {max_wait_seconds} 秒）")
    print("=" * 70)
    
    page.goto('https://www.yeahpromos.com/index/login/login')
    
    for i in range(max_wait_seconds):
        time.sleep(1)
        
        if (i + 1) % 10 == 0:
            print(f"等待登录... ({i + 1}/{max_wait_seconds}秒)")
    ...
    print(f"\n⚠️ {max_wait_seconds} 秒内未检测到登录，尝试继续...")
    return True
...
def main():
    print("=" * 70)
    print("YP US 商户商品批量抓取工具（自动检测登录版本）")
    print("=" * 70)
    ...
    with sync_playwright() as p:
        ...
        wait_for_login(page, max_wait_seconds=180)
