...
def log(tag, msg):
    ...
    safe_msg = msg.encode('ascii', 'ignore').decode('ascii') if isinstance(msg, str) else str(msg)
...
def parse_html_fallback(page, mid, merchant_name):
    ...
    if not sess_cookie:
        ...
        link_list = [l.replace("&amp;","&") for l in re.findall(r"ClipboardJS\.copy\('([^']+)'\)", html)]
        for i, asin in enumerate(asin_list):
            products.append({"asin":asin,"product_name":"","category":"","commission":"",
                             "price":"","tracking_link":link_list[i] if i<len(link_list) else "",
...
def main():
    ...
    if single_mid:
        ...
        try:
            ...
            cur.execute("SELECT merchant_name FROM yp_merchants WHERE merchant_id=%s LIMIT 1", (single_mid,))
            row = cur.fetchone()
            cur.close()
        ...
        pending = [m for m in all_merchants
                   if m['mid'] not in completed_mids and m['mid'] not in failed_mids]
        log("MAIN", f"总商户: {len(all_merchants)} | 已完成: {len(completed_mids)} | 失败: {len(failed_mids)} | 待处理: {len(pending)}")
    ...
    with sync_playwright() as p:
        ...
        if browser is None:
            ...
            candidates += _glob.glob(r"C:\Users\wuhj\AppData\Local\**\chrome.exe", recursive=True)[:3]
            chrome_exe = next((c for c in candidates if os.path.isfile(c)), None)
            if not chrome_exe:
                log("MAIN", "❌ 未找到 Chrome，请手动运行: chrome.exe --remote-debugging-port=9222 --user-data-dir=\"C:\\Users\\wuhj\\Chrome_Debug\"")
                db_conn.close()
                return
            ...
            browser = _try_connect_chrome(p)
            if browser is None:
                log("MAIN", "❌ Chrome 调试模式启动后仍无法连接，请确认已登录 YP 平台后重试")
                db_conn.close()
                return
    ...
    log("MAIN", f"耗时: {elapsed_total//3600}h{elapsed_total%3600//60}m{elapsed_total%60}s")
